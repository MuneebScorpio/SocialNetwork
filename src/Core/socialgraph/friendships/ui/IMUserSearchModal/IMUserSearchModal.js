...
function IMUserSearchModal(props) {
  ...
  }, [currentUser.id, followEnabled, store]);

  useEffect(() => {
    if (reduxUsers) {
      updateFilteredFriendships();
    }
  }, [reduxUsers, updateFilteredFriendships]);

  useEffect(() => {
    if (friendships) {
      updateFilteredFriendships();
    }
  }, [friendships, updateFilteredFriendships]);
  ...
  const updateFilteredFriendships = useCallback(
