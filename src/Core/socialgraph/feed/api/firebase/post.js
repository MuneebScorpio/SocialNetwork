...
const removePostFromAllTimelines = async post => {
  ...
  const unsubscribe = friendshipAPIManager.subscribeToInboundFriendships(
    post.authorID,
    inboundFriendships => {
...
const removePostFromAllFriendsTimelines = async post => {
  // We fetch all users who follow the author of the post and update their timelines
  const unsubscribeInbound = friendshipAPIManager.subscribeToInboundFriendships(
    post.authorID,
    inboundFriendships => {
      const unsubscribeOutbound =
        friendshipAPIManager.subscribeToInboundFriendships(
          post.authorID,
          outboundFriendships => {
            const inboundUserIDs = inboundFriendships.map(
              friendship => friendship.id,
